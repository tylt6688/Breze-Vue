				...
				<el-descriptions title="个人资料" :column="3" style="margin:0 auto;width:70%" border>
					...
					<el-descriptions-item>
						...
						<template :slot-scope="userInfo">
							<el-tag style="margin: 3px;" size="small" v-for="(item,index) in userInfo.roles"
								:key="index">
								{{item.name}}</el-tag>
						</template>
					</el-descriptions-item>
			...
			<el-tab-pane label="安全隐私">
				<el-form ref="form" label-width="100px">
					...
					<el-form-item label="登录邮件提醒">
						<el-switch @change="changeLoginWarn" v-model="loginWarn" active-color="#13ce66"
	...
	export default {
		...
		methods: {
			...
			changeLoginWarn(e) {
				...
				this.$axios.post('/sys/user/updateloginwarn?loginwarn=' + loginwarn + '&id=' + this.userInfo.id)
					.then(res => {
						this.$message({
							showClose: true,
							duration: 2000,
			...
			submitUserInfoForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$axios.post('/sys/user/updateuserinfo', this.editForm)
							.then(res => {
								this.$message({
									showClose: true,
									duration: 2000,
			...
			uploadAvatar(param) {
				...
				this.$axios
					.post("/sys/user/updateavatar", formData, {
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						...
						this.$axios.post('/sys/user/updatepassword', this.passForm).then(res => {
							...
							that.$alert(res.data.result.data.message, '提示', {
