<template>
	<div class="page">
		<el-form :inline="true">
			...
			<el-form-item>
				<el-button type="primary" @click="dialogVisible = true" v-if="hasAuth('sys:user:insert')"
					icon="el-icon-plus">新增用户</el-button>
			</el-form-item>

			<el-form-item>
				<el-popconfirm title="确定将已选择用户批量删除吗?" @confirm="delHandle(null)">
					<el-button type="danger" icon="el-icon-delete" slot="reference" :disabled="delBtlStatu"
			...
			<el-form-item>
				<el-button type="warning" icon="el-icon-connection" @click="rolemoreHandle"
					:disabled="permMoreBtlStatu">
					批量授权</el-button>
			</el-form-item>
		...
		<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" border stripe
			...
			<el-table-column prop="statu" label="状态" align="center" width="80">
				<template slot-scope="scope">
			...
			<el-table-column prop="created" label="创建时间" align="center" :formatter="formatDate" width="160">
			</el-table-column>
			...
			<el-table-column prop="icon" label="操作" width="330">
				...
				<template slot-scope="scope">
					...
					<el-button type="text" @click="repassHandle(scope.row.id, scope.row.username)"
						icon="el-icon-refresh-left">重置密码</el-button>
					<el-divider direction="vertical"></el-divider>
		...
		<el-dialog title="新增或编辑用户" :visible.sync="dialogVisible" width="600px" :before-close="handleClose">
			<el-form :model="editForm" :rules="editFormRules" ref="editForm">
				<el-form-item label="登录用户名" prop="username" label-width="100px">
					...
					<el-alert title="用户名可为统一工号，新增用户初始密码为:123456" :closable="false" type="info"
		...
		<el-dialog title="批量新增用户" :visible.sync="moredialogVisible" width="600px" :before-close="morehandClose">
			...
			<el-upload ref="upload" class="downloadModel" action="url" drag :http-request="uploadExcel" multiple
				accept=".xls,.xlsx" :limit="1" :auto-upload="uploadStatu">
				<i class="el-icon-upload"></i>
				<div class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
				<div class="el-upload__tip" slot="tip">只能上传xlsx文件,请按照模板文件上传</div>
			</el-upload>
			<div slot="footer" class="dialog-footer">
		...
		<el-dialog title="批量分配角色" :visible.sync="roleMoreDialogFormVisible" width="600px">
			...
			<el-form :model="roleForm">
				<el-tree :data="roleTreeData" show-checkbox ref="roleMoreTree" :check-strictly=checkStrictly
					node-key="id" :default-expand-all=true :props="defaultProps">
				</el-tree>
			</el-form>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 获取已存在权限 Start
			getRoleTree() {
				this.$axios.get("/sys/role/select").then(res => {
			...
			// 获取用户列表 Start
			getUserList() {
				this.$axios.get("/sys/user/select", {
					params: {
			...
			handleSelectionChange(val) {
				...
				this.permMoreBtlStatu = val.length == 0;
			},
			// 获取选中的行数据 End
			...
			uploadExcel(param) {
				...
				this.$axios
					.post("/sys/user/uploadexcel", formData, {
			...
			// 导出当前所有用户Excel Start
			exportExcel() {
				this.$axios.get("/sys/user/downloadexcel", {
					...
					tmpLink.setAttribute("download", "超融合一体化教学平台用户文件.xlsx");
			...
			// 导出Excel上传模板 Start
			exportModelExcel() {
				this.$axios.get("/sys/user/downloadmodelexcel", {
					responseType: "arraybuffer"
				}).then(res => {
					const blob = new Blob([res.data]);
					const bloburl = window.URL.createObjectURL(blob);
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$axios.post('/sys/user/' + (this.editForm.id ? 'update' : 'insert'), this.editForm)
							.then(res => {
								this.$message({
									showClose: true,
									duration: 2000,
			...
			submitMoreRoleHandle(formName) {
				...
				this.$axios.post('/sys/user/rolepermmore?userIds=' + userIds, roleIds).then(res => {
					this.$message({
						showClose: true,
						duration: 2000,
						message: '批量分配成功',
			...
			submitRoleHandle(formName) {
				...
				console.log(roleIds)
				this.$axios.post('/sys/user/roleperm/' + this.roleForm.id, roleIds).then(res => {
					this.$message({
						showClose: true,
						duration: 2000,
						message: '操作成功',
			...
			repassHandle(id, username) {
				this.$confirm('将重置用户[ ' + username + ' ]的密码, 是否继续?', '重置密码', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
					this.$axios.post("/sys/user/resertpassword", id).then(res => {
						this.$message({
							showClose: true,
							duration: 2000,
							message: '重置密码成功',
