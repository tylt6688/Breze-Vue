<template>
  <div class="page">
    <el-tabs v-model="activeName" tab-position="top" @tab-click="handleClick">
      <el-tab-pane label="轮播图管理" name="first">
        <el-header>
          <el-button
            type="primary"
            @click="bannerDialogVisible = true"
            ...
            <el-table-column prop="icon" label="操作" width="150">
              <template slot-scope="scope">
                ...
                <template>
                  <el-popconfirm
                    title="确定删除此轮播图吗?"
                    @confirm="delHandle(scope.row.url)"
                  >
                    <el-button
                      type="text"
                      slot="reference"
      ...
      <el-tab-pane label="通知公告管理" name="second">
        <el-header>
          ...
          <el-button
            type="success"
            @click="commonEditDialogVisible = true"
        ...
        <Context
          :tableData="notifyTableData"
          :total="total"
          :current="current"
          :size="size"
        ></Context>
        <el-pagination
          @size-change="notifyHandleSizeChange"
          @current-change="notifyHandleCurrentChange"
      ...
      <el-tab-pane label="学生天地管理" name="fourth">
        ...
        <el-pagination
          @size-change="presenceHandleSizeChange"
          @current-change="presenceHandleCurrentChange"
          layout="total, sizes, prev, pager, next, jumper"
          :page-sizes="[10, 20, 50, 100]"
          :current-page="current"
          :page-size="size"
      ...
      <el-tab-pane label="思政教育管理" name="fiveth">
        ...
        <el-pagination
          @size-change="thoughtHandleSizeChange"
          @current-change="thoughtHandleCurrentChange"
          layout="total, sizes, prev, pager, next, jumper"
          :page-sizes="[10, 20, 50, 100]"
          :current-page="current"
          :page-size="size"
    ...
    <el-dialog
      title="输入标题"
      :visible.sync="commonEditDialogVisible"
      :before-close="commonEditHandleClose"
    >
      <el-form
        ref="commonEditForm"
        :model="commonEditForm"
        :rules="commonEditFormRules"
        size="medium"
        label-width="100px"
      >
        <el-form-item label="请输入标题" prop="title" autocomplete="off">
          <el-input
            v-model="commonEditForm.title"
            placeholder="请输入请输入标题"
            clearable
            :style="{ width: '100%' }"
          >
          </el-input>
          <el-alert
            title="为保护数据防止误操作，请在此复制相应标题进行操作确认"
            :closable="false"
            type="info"
      ...
      <div slot="footer">
        <el-button v-if="delflag" type="primary" @click="delNotify"
          >删除</el-button
        >
        <el-button v-else type="primary" @click="submitCommonEdit"
          >确定</el-button
        >

        <el-button @click="commonEditResetForm('commonEditForm')"
          >取消</el-button
        >
      </div>
    </el-dialog>
    ...
    <el-dialog
      ...
      :before-close="notifyHandleClose"
    >
      <el-form
        ref="notifyForm"
        :model="notifyForm"
      ...
      >
        ...
        <el-form-item label="请输入内容" prop="content">
          <tinymce
            ...
            placeholder="请输入请输入内容"
            :height="420"
          ></tinymce>
        </el-form-item>
      </el-form>
      <div slot="footer">
        <el-button type="primary" @click="submitNotifyForm('notifyForm')"
          >确定</el-button
        >
    ...
    >
      <el-form>
        ...
        <el-form-item label="排序" label-width="100px">
          <el-input-number v-model="ordernum" :min="1" label="序号"
            >1</el-input-number
          >
        </el-form-item>
...
import indexManage from "@/api/sys/indexManage";
