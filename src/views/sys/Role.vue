<template>
	<div class="page">
		...
		<el-form :inline="true">
			...
			<el-form-item>
				<el-popconfirm title="确定删除已选项吗？" @confirm="delHandle(null)">
					<el-button type="danger" slot="reference" :disabled="delBtlStatu" icon="el-icon-delete">批量删除
					</el-button>
				</el-popconfirm>
			</el-form-item>
		</el-form>
		...
		<el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" border stripe
			...
			<el-table-column prop="statu" label="状态" align="center" width="160">
			...
			<el-table-column prop="created" label="创建时间" align="center" :formatter="formatDate" show-overflow-tooltip>
			</el-table-column>
...
<script>
	...
	export default {
		...
		created() {
			this.getRoleList();
			this.$axios.get('/sys/menu/select').then(res => {
		...
		methods: {
			...
			// 获取角色列表数据
			getRoleList() {
				this.$axios.get("/sys/role/select", {
					params: {
			...
			submitForm(formName) {
				this.$refs[formName].validate((valid) => {
					if (valid) {
						this.$axios.post('/sys/role/' + (this.editForm.id ? 'update' : 'insert'), this.editForm)
							.then(res => {
								this.$message({
									showClose: true,
									duration: 2000,
